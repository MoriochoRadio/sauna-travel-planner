...
향후 사용자가 정적판 일정 v2를 로그인 플랜으로 가져오는 기능을 추가하려면, 서버 측에서 로그인 사용자 확인, 등록 장소 검증, 240자 메모 제한, 최대 항목 수, 중복 제거를 다시 수행해야 한다. 브라우저에서 만든 복원 코드를 신뢰해 사용자 ID·관리 상태·공유 토큰·예산을 채우거나 덮어쓰면 안 된다.
